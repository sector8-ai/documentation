...
**Current:** [TypeScript examples v2](/downloads/onboarding/v2/typescript.json) · [Python examples v2](/downloads/onboarding/v2/python.json)

v2 is Windows `cp1252`-safe (ASCII console output) so a successful live walkthrough exits `0`. [v1](/downloads/onboarding/v1/) remains available but is not the acceptance path.
...
<Steps>
  ...
  <Step title="3. Download public examples">
    ...
    **Typescript (bash)**
    ...
    ```bash
    curl -fsSL -o typescript.json https://sector8.mintlify.app/downloads/onboarding/v2/typescript.json
    ...
    **Typescript (powershell)**
    ...
    ```powershell
    Invoke-WebRequest -UseBasicParsing -OutFile typescript.json https://sector8.mintlify.app/downloads/onboarding/v2/typescript.json
    node -e "const fs=require('node:fs'); const b=JSON.parse(fs.readFileSync('typescript.json','utf8')); for(const n of ['onboarding-verify.js','refund-pilot-core.js','refund-pilot-walkthrough.js']) fs.writeFileSync(n,b.files[n],{flag:'wx'});"
    ```
    ...
    **Python (bash)**
    ...
    ```bash
    curl -fsSL -o python.json https://sector8.mintlify.app/downloads/onboarding/v2/python.json
    ...
    **Python (powershell)**
    ...
    ```powershell
    Invoke-WebRequest -UseBasicParsing -OutFile python.json https://sector8.mintlify.app/downloads/onboarding/v2/python.json
    python -c "import json,pathlib; b=json.loads(pathlib.Path('python.json').read_text(encoding='utf-8')); names=('onboarding_verify.py','refund_pilot_core.py','refund_pilot_walkthrough.py'); [pathlib.Path(n).open('x',encoding='utf-8').write(b['files'][n]) for n in names]"
    ```
