...
| **Art. 14** | Human oversight | `REVIEW` decisions pause for human approval before execution; unanswered review items can auto-deny (fail-closed). |
...
- **Art. 14** support means `REVIEW` can pause risky actions for human approval before dispatch.

Honor `ALLOW`, `DENY`, and `REVIEW` at the dispatch boundary. Missing, unknown, or failed evaluate results should fail closed on the routed path.
