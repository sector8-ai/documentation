...
tool dispatch, then honor the returned decision.
...
The container pattern can keep the base image unchanged and add Sector8 in your approved image layer. Your runtime still needs an evaluate-before-dispatch hook.
...
Replace direct tool dispatch with an evaluate-before-dispatch hook. The adapter calls
`POST /api/v1/evaluate` and must not execute the tool unless the decision is `ALLOW`.
...
Route every governed tool call through `adapter.execute(...)` before local dispatch:
...
If you call REST directly instead of the adapter, honor the decision the same way as the [Quickstart](/guides/quickstart):
...
<Warning>
Fail closed if evaluate fails, times out, or returns an unknown decision. Do not execute the proposed action by default. Fail-closed applies to the routed evaluate path; it does not stop actions on bypassed or unconfigured tool paths.
</Warning>
...
- Your runtime calls `/api/v1/evaluate` before local dispatch
