...
When a tool call is routed through `sector8-guard`, the caller-visible decision is `ALLOW` or `DENY`. On this connector path a borderline action can be held for a human before that decision is returned; if nobody answers, it is denied.
...
On this connector path, a borderline action can be held for a human before `ALLOW` or `DENY` is returned. If nobody answers, the connector denies. That hold is connector behaviour, not a third public `decision` value. REST evaluate does not wait.
