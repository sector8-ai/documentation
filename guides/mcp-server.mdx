...
When a tool call is routed through `sector8-guard`, Sector8 returns `ALLOW`, `DENY`, or `REVIEW` using the same admission and policy path as the REST evaluate API.
...
This is not a separate policy engine. The MCP server uses the same decision contract, reason codes, and evidence fields as the rest of Sector8.
...
Honor the returned decision before any side effect starts. Policy, telemetry, and decision evidence are unified with the REST evaluate path.
