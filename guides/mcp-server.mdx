...
description: Govern Claude Code and MCP-connected agent tool calls with evaluate-before-dispatch decisions and fail-closed runtime behavior.
---

## What it does

The Guard MCP Server connects Claude Code (and other MCP-compatible clients) to Sector8 so proposed tool calls are evaluated before they execute.
...
| --- | --- |
| `ALLOW` | The MCP server may execute the proposed tool call |
| `DENY` | The MCP server must block the proposed tool call |
...
If evaluation fails while the tool call is routed through `sector8-guard`, treat the outcome as fail-closed: the tool should not execute.
...
- `echo hello` -> `ALLOW`
- `rm -rf /` -> `DENY`
- unknown or missing decisions -> fail closed; do not execute
